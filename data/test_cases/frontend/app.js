...
async function runTest(testId, options = {}) {
    ...
    if (result.success) {
        ...
        const finalStatus = result.data?.status || 'completed';
        updateStatusUI({ status: finalStatus });
    } else {
        // Update status to failed on error
        updateStatusUI({ status: 'failed' });
    }

    // Refresh statistics after test completion
    await getStatistics();
...
async function resumeExecution() {
    ...
    if (result.success) {
        ...
        // Update status to running (resumed)
        updateStatusUI({ status: 'running' });
    } else {
...
/* ═══════════════════════════════════════════════════════════════ */
/* CROPPED VERIFICATION IMAGES APIs (for Partial Image Verification) */
/* ═══════════════════════════════════════════════════════════════ */

async function previewCropRegion() {
    const x1 = parseInt(document.getElementById('crop-x1').value) || 0;
    const y1 = parseInt(document.getElementById('crop-y1').value) || 0;
    const x2 = parseInt(document.getElementById('crop-x2').value) || 0;
    const y2 = parseInt(document.getElementById('crop-y2').value) || 0;

    if (x2 <= x1 || y2 <= y1) {
        showNotification('Invalid coordinates: x2 must be > x1 and y2 must be > y1', 'warning');
        return;
    }

    try {
        addLog('info', `Previewing crop: (${x1},${y1}) to (${x2},${y2})`);

        const response = await fetch(`${state.settings.apiUrl}/api/verification/preview-crop`, {
            method: 'POST',
        ...
        const previewInfo = document.getElementById('crop-preview-info');

        previewImage.src = imageUrl;
        previewInfo.textContent = `Size: ${x2 - x1}x${y2 - y1} pixels`;
        previewContainer.style.display = 'block';

        addLog('success', `Crop preview loaded: ${x2 - x1}x${y2 - y1}`);

    } catch (error) {
        console.error('Preview crop error:', error);
        addLog('error', `Failed to preview crop: ${error.message}`);
        showNotification(`Preview failed: ${error.message}`, 'error');
    }
}

async function captureCroppedVerificationImage() {
    const imageName = document.getElementById('crop-image-name').value.trim();
    const x1 = parseInt(document.getElementById('crop-x1').value) || 0;
    const y1 = parseInt(document.getElementById('crop-y1').value) || 0;
    const x2 = parseInt(document.getElementById('crop-x2').value) || 0;
    const y2 = parseInt(document.getElementById('crop-y2').value) || 0;
    ...
    if (!imageName) {
        showNotification('Please enter a name for the cropped image', 'warning');
        return;
    }
    ...
    try {
        addLog('info', `Capturing cropped verification image: ${imageName}`);

        const result = await apiRequest('/api/verification/capture-cropped', 'POST', {
            image_name: imageName,
            x1, y1, x2, y2,
            description: description || null
        });
        ...
        if (result.success) {
            addLog('success', `Cropped verification image captured: ${result.image_name}`);
            showNotification(`Cropped image "${result.image_name}" saved`, 'success');

            // Clear inputs
            document.getElementById('crop-image-name').value = '';
            document.getElementById('crop-description').value = '';
            document.getElementById('crop-preview-container').style.display = 'none';

            // Refresh cropped images list if visible
            const listContainer = document.getElementById('cropped-images-list-container');
            if (listContainer.style.display === 'block') {
                await viewCroppedImages();
            }
        }
    } catch (error) {
        console.error('Capture cropped image error:', error);
        addLog('error', `Failed to capture cropped image: ${error.message}`);
    }
}

async function viewCroppedImages() {
    try {
        const result = await apiRequest('/api/verification/cropped-images');

        if (!result.success) {
            showNotification('Failed to load cropped images', 'error');
            return;
        }

        const container = document.getElementById('cropped-images-list-container');
        const listElement = document.getElementById('cropped-images-list');

        // Hide full images list
        document.getElementById('verification-images-list-container').style.display = 'none';

        if (!result.images || result.images.length === 0) {
            listElement.innerHTML = '<div style="padding: 8px; color: #888; font-size: 11px;">No cropped images captured yet</div>';
            container.style.display = 'block';
            addLog('info', 'No cropped verification images found');
            return;
        }

        // Build list HTML with clickable coordinates and delete button
        listElement.innerHTML = result.images.map(img => {
            const date = img.created_at ? new Date(img.created_at).toLocaleDateString() : 'Unknown';
            const coords = img.crop_coords || {};
            const coordsStr = `(${coords.x1 || 0},${coords.y1 || 0}) to (${coords.x2 || 0},${coords.y2 || 0})`;

            return `
                <div class="cropped-image-item" data-image-name="${img.clean_name}">
                    <div class="cropped-image-header">
                        <span class="cropped-image-name">${img.name}</span>
                        <div class="cropped-image-actions">
                            <span class="cropped-image-coords"
                                  onclick="fillCropCoords(${coords.x1 || 0}, ${coords.y1 || 0}, ${coords.x2 || 0}, ${coords.y2 || 0}); event.stopPropagation();"
                                  title="Click to fill form with these coordinates">
                                ${coordsStr}
                            </span>
                            <button
                                class="verification-image-delete"
                                onclick="deleteCroppedImage('${img.clean_name}'); event.stopPropagation();"
                    ...
                    ${img.description ? `<div class="cropped-image-desc">${img.description}</div>` : ''}
                    <div class="cropped-image-date">${date}</div>
                </div>
            `;
        }).join('');

        // Add click handlers for preview
        document.querySelectorAll('.cropped-image-item').forEach(item => {
            item.addEventListener('click', () => {
                const imageName = item.getAttribute('data-image-name');
                previewCroppedImage(imageName);
            });
        });

        container.style.display = 'block';
        addLog('success', `Loaded ${result.images.length} cropped verification images`);

        // Update device info
        updateVerificationDeviceInfo(result.device_id, result.resolution);

    } catch (error) {
        console.error('View cropped images error:', error);
        addLog('error', `Failed to load cropped images: ${error.message}`);
    }
}

function fillCropCoords(x1, y1, x2, y2) {
    document.getElementById('crop-x1').value = x1;
    document.getElementById('crop-y1').value = y1;
    document.getElementById('crop-x2').value = x2;
    document.getElementById('crop-y2').value = y2;
    showNotification(`Coordinates filled: (${x1},${y1}) to (${x2},${y2})`, 'success');
}

async function previewCroppedImage(imageName) {
    try {
        const imageUrl = `${state.settings.apiUrl}/api/verification/cropped-image/${encodeURIComponent(imageName)}`;

        // Show in modal
        const modal = document.getElementById('verification-image-preview-modal');
        const img = document.getElementById('verification-preview-image');
        const nameLabel = document.getElementById('verification-preview-name');

        img.src = imageUrl;
        nameLabel.textContent = imageName;
        modal.style.display = 'flex';

        addLog('info', `Previewing cropped verification image: ${imageName}`);

        // Auto-close after 3 seconds (same as full images)
        setTimeout(() => {
            modal.style.display = 'none';
        }, 3000);

    } catch (error) {
        console.error('Preview cropped image error:', error);
        addLog('error', `Failed to preview image: ${error.message}`);
    }
}

async function deleteCroppedImage(imageName) {
    if (!confirm(`Delete cropped verification image "${imageName}"?`)) {
        return;
    }

    try {
        addLog('info', `Deleting cropped verification image: ${imageName}`);

        const result = await apiRequest(`/api/verification/cropped-image/${encodeURIComponent(imageName)}`, 'DELETE');

        if (result.success) {
            addLog('success', `Cropped verification image deleted: ${imageName}`);
            showNotification(`Cropped image "${imageName}" deleted`, 'success');

            // Refresh list
            await viewCroppedImages();
        }
    } catch (error) {
        console.error('Delete cropped image error:', error);
        addLog('error', `Failed to delete cropped image: ${error.message}`);
    }
}
...
function updateAgentStatus(data) {
    ...
    if (statusDot && statusText) {
        ...
        statusText.textContent = formatStatusText(state.agentStatus);
    }

    // Update dashboard card status
    const cardStatus = document.getElementById('card-agent-status');
    if (cardStatus) {
        cardStatus.textContent = formatStatusText(state.agentStatus);
        cardStatus.className = 'card-value';
        cardStatus.classList.add(`status-${state.agentStatus}`);
    }
...
function updateStatusUI(statusData) {
    if (statusData.status) {
        ...
        if (statusDot && statusText) {
            ...
            statusText.textContent = formatStatusText(statusData.status);
        }

        // Update dashboard card status
        const cardStatus = document.getElementById('card-agent-status');
        if (cardStatus) {
            cardStatus.textContent = formatStatusText(statusData.status);
            // Update card styling based on status
            cardStatus.className = 'card-value';
            cardStatus.classList.add(`status-${statusData.status}`);
        }
    }
    ...
    // Update statistics if included in status data
    if (statusData.tests_executed !== undefined) {
        updateStatistics({
            tests_executed: statusData.tests_executed,
            tests_passed: statusData.tests_passed || 0,
            tests_failed: statusData.tests_failed || 0
        });
    }
}
...
function handleHITLRequest(data) {
    ...
    // Update status UI using centralized function
    updateStatusUI({ status: 'waiting_hitl' });
}
...
window.deleteCroppedImage = deleteCroppedImage;
window.fillCropCoords = fillCropCoords;
...
function setupEventListeners() {
    ...
    // Cropped Verification Images Event Listeners
    const previewCropBtn = document.getElementById('preview-crop-btn');
    if (previewCropBtn) {
        previewCropBtn.addEventListener('click', async () => {
            await previewCropRegion();
        });
    }

    const captureCroppedBtn = document.getElementById('capture-cropped-btn');
    if (captureCroppedBtn) {
        captureCroppedBtn.addEventListener('click', async () => {
            await captureCroppedVerificationImage();
        });
    }

    const viewCroppedImagesBtn = document.getElementById('view-cropped-images-btn');
    if (viewCroppedImagesBtn) {
        viewCroppedImagesBtn.addEventListener('click', async () => {
            await viewCroppedImages();
        });
    }
...
/* ═══════════════════════════════════════════════════════════════ */
/* SSIM SEARCH FUNCTIONS */
/* ═══════════════════════════════════════════════════════════════ */

// Store all loaded results for filtering
let ssimAllResults = {
    success: [],
    failed: [],
    all: []
...
// Search SSIM Results
async function searchSSIMResults() {
    const testId = document.getElementById('ssim-search-test-id').value.trim();
    const stepInput = document.getElementById('ssim-search-step').value.trim();
    const status = document.getElementById('ssim-search-status').value;
    const dateInput = document.getElementById('ssim-search-date').value;

    // Build query params
    const params = new URLSearchParams();
    if (testId) params.append('test_id', testId);
    if (stepInput) params.append('step', stepInput);
    if (status && status !== 'all') params.append('category', status === 'success' ? 'success' : 'error');
    ...
    try {
        const response = await apiRequest(`/api/verification/search?${params.toString()}`);

        if (response.success) {
            const results = response.data.results || [];

            // Show search info
            showSearchInfo(results.length, { testId, stepInput, status, dateInput });
            ...
            if (results.length === 0) {
                ...
                renderSearchResults(results, { testId, stepInput });
            }

            // Clear button states
            document.querySelectorAll('.ssim-buttons-group .btn').forEach(b => b.classList.remove('active'));

            addLog('info', `SSIM search found ${results.length} results`);
        } else {
            list.innerHTML = `<div style="padding: 20px; text-align: center; color: var(--status-error);">
                Search failed: ${response.message || 'Unknown error'}
            </div>`;
        }
    } catch (error) {
        console.error('SSIM search error:', error);
        list.innerHTML = `<div style="padding: 20px; text-align: center; color: var(--status-error);">
...
// Show search info with filter tags
function showSearchInfo(count, filters) {
    const infoEl = document.getElementById('ssim-search-info');
    const countEl = document.getElementById('ssim-search-count');
    const filtersEl = document.getElementById('ssim-search-filters');

    countEl.textContent = count;

    // Build filter tags
    const tags = [];
    if (filters.testId) tags.push(`<span class="filter-tag">ID: ${filters.testId}</span>`);
    if (filters.stepInput) tags.push(`<span class="filter-tag">Step: ${filters.stepInput}</span>`);
    if (filters.status && filters.status !== 'all') tags.push(`<span class="filter-tag">${filters.status}</span>`);
    if (filters.dateInput) tags.push(`<span class="filter-tag">${filters.dateInput}</span>`);

    filtersEl.innerHTML = tags.length > 0 ? '| Filters: ' + tags.join(' ') : '';
    infoEl.style.display = 'flex';
}

// Render search results with highlighting
function renderSearchResults(results, searchTerms) {
    const list = document.getElementById('ssim-results-list');
    ...
    list.innerHTML = results.map(r => {
        ...
        // Highlight matching test ID
        let testIdDisplay = r.test_id;
        if (searchTerms.testId && r.test_id.toLowerCase().includes(searchTerms.testId.toLowerCase())) {
            const regex = new RegExp(`(${escapeRegex(searchTerms.testId)})`, 'gi');
            testIdDisplay = r.test_id.replace(regex, '<span class="highlight">$1</span>');
        }

        return `
            <div class="ssim-result-item ${statusClass} search-match" onclick="viewSSIMComparison('${r.result_id}')">
                <div class="ssim-result-header">
                    <span class="ssim-result-test-id">${icon} ${testIdDisplay}</span>
                    <span class="ssim-result-score ${scoreClass}">${r.ssim_score.toFixed(4)}</span>
                </div>
                <div class="ssim-result-step">Step ${r.step_number}${r.step_description ? ': ' + r.step_description : ''}</div>
                <div class="ssim-result-meta">${date}</div>
...
function clearSSIMSearch() {
    document.getElementById('ssim-search-test-id').value = '';
    document.getElementById('ssim-search-step').value = '';
    document.getElementById('ssim-search-status').value = 'all';
    document.getElementById('ssim-search-date').value = '';

    // Hide search info
    document.getElementById('ssim-search-info').style.display = 'none';

    // Hide results container
    document.getElementById('ssim-results-container').style.display = 'none';

    // Clear button states
    document.querySelectorAll('.ssim-buttons-group .btn').forEach(b => b.classList.remove('active'));

    addLog('info', 'SSIM search filters cleared');
}

// Quick search by test ID (can be called from other parts of the app)
async function quickSearchSSIM(testId, status = 'all') {
    document.getElementById('ssim-search-test-id').value = testId;
    document.getElementById('ssim-search-status').value = status;
    await searchSSIMResults();
}

// Initialize SSIM search event listeners
function initSSIMSearch() {
    const searchBtn = document.getElementById('ssim-search-btn');
    const clearBtn = document.getElementById('ssim-clear-search-btn');
    const testIdInput = document.getElementById('ssim-search-test-id');

    if (searchBtn) {
        searchBtn.addEventListener('click', searchSSIMResults);
    }

    if (clearBtn) {
        clearBtn.addEventListener('click', clearSSIMSearch);
    }

    // Allow Enter key to trigger search
    if (testIdInput) {
        testIdInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                searchSSIMResults();
            }
        });
    ...
    const stepInput = document.getElementById('ssim-search-step');
    if (stepInput) {
...
window.searchSSIMResults = searchSSIMResults;
window.clearSSIMSearch = clearSSIMSearch;
window.quickSearchSSIM = quickSearchSSIM;
